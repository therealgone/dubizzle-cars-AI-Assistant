...
### Agent tools & API
The model has eight tools: `search_cars`, `select_car`, `manage_favorite` (add / remove / list), `search_history` (the user's own past selections and favorites), `get_chat_history` (summaries, optionally for a given date), `manage_booking` (create / reschedule / cancel / list), `qualify_lead`, and `compare_cars`. Small single-purpose tools keep each call auditable and let the system prompt attach specific rules to specific tools. The FastAPI backend exposes `POST /chat` (the agent loop), `POST /select_car` and `POST /manage_favorite` (direct UI actions that skip the LLM), `POST /new_session`, `GET /search`, and two dev-only endpoints, `GET /dev/session_cache` and `GET /dev/long_term_memory`. Inventory retrieval is available two ways: as the `search_cars` tool the model calls inside `/chat`, and as `GET /search`, which runs the same search function directly from structured parameters (`make`, `model`, `trim`, `year`, `body_type`, `color`, `description`, `keywords`, `semantic_query`, `top_k`) with no LLM involved. The endpoint is stateless, so it never touches a user's session or history.
...
### Lead qualification
Leads are recorded in two ways that share one duplicate check. First, once the model knows the user's budget and at least one other preference (make, body type, and so on), it can call `qualify_lead` straight away. Second, and more reliably, every time a chat summary is saved (every 2 exchanges) the same LLM call also pulls the user's current budget, preferences and notes out of the conversation, so leads don't depend on the model remembering to call a tool. Either way, a row — timestamp, username, price range, preferences, notes — is appended to `data/leads.csv` only when the need differs from that user's last recorded row, so the file logs changes rather than repeats, and small talk with no budget or preference adds nothing. The log is append-only and shared across users, so a user whose needs evolve appears multiple times, like a real lead-tracking feed. The file is created with its header row when the backend starts, not the first time a lead is recorded.
